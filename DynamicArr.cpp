...
template <class T>
DynamicArr<T>::DynamicArr() : size_(0), capacity_(10), arr_(new T[capacity_]) {
}
...
template <class T>
DynamicArr<T>::DynamicArr(const DynamicArr<T>& rhs) : size_(rhs.size_), capacity_(rhs.capacity_), arr_(new T[capacity_]) {
    for (size_t i = 0; i < size_; ++i) {
        arr_[i] = rhs.arr_[i];
    }
}
...
template <class T>
DynamicArr<T>::DynamicArr(DynamicArr<T>&& rhs) noexcept : size_(rhs.size_), capacity_(rhs.capacity_), arr_(rhs.arr_) {
    rhs.arr_ = nullptr;  // Чтобы избежать удаления массива при вызове деструктора
    rhs.size_ = 0;
    rhs.capacity_ = 0;
}
